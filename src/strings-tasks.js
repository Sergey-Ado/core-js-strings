...
function sumOfCodes(str) {
  if (!str) return 0;
  return str.split('').reduce((sum, s) => sum + s.charCodeAt(0), 0);
}
...
function countVowels(str) {
  ...
  return str.split('').reduce((sum, s) => sum + vowels.includes(s), 0);
}
...
function isPalindrome(str) {
  const cutStr = str.toLowerCase().replace(/\W/g, '');
  return cutStr === cutStr.split('').reverse().join('');
}
...
function extractNameFromTemplate(value) {
  return value.match(/^Hello, ([\w\s]*)!$/)[1];
}
